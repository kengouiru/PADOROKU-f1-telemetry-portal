 ...
 * 1. 👥 チーム別グループ表示 (Team Grouped View: 10 F1 Teams + Historic Legends)
 * 2. 📋 全ドライバー一覧 (Flat Grid with Status & Team Filters)
 * 3. 👑 歴代レジェンド (Hall of Fame Legends Showcase)
 * Includes Search Filter, Quick Stats, and DriverDetailModal.
 */
...
export default function DriversHub({
  ...
  }, [searchQuery, driverStatusFilter, driverTeamFilter]);
  ...
  return (
    <div className="flex flex-col gap-4 animate-fade-in">
      {/* ── Top Bar: View Mode Switcher & Summary / Reset ── */}
      <div className="bg-slate-950/70 border border-white/10 rounded-2xl p-4 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 self-end md:self-auto flex-shrink-0">
          <span className="text-xs font-mono font-bold bg-slate-900 px-3 py-1.5 rounded-xl border border-white/10 text-sky-400">
            {viewMode === 'grouped' && '全10チーム (現役20名 + レジェンド)'}
            {viewMode === 'flat' && `該当: ${filteredDrivers.length} / ${KNOWLEDGE_DRIVERS.length} 名`}
            {viewMode === 'legends' && `殿堂レジェンド: ${legendDrivers.length} 名`}
          </span>
      ...
      {viewMode === 'grouped' && (
        <div className="flex flex-col gap-6">
          ...
          {(!searchQuery.trim() ||
            ...
            <div className="bg-gradient-to-r from-amber-950/30 via-slate-950/80 to-amber-950/30 border border-amber-500/30 rounded-2xl p-4 sm:p-5 shadow-xl mt-2">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 mb-3.5 border-b border-amber-500/20 p-3 rounded-xl bg-amber-500/10 border-l-4 border-l-amber-400">
                ...
                <div className="text-xs font-mono text-amber-300/80 bg-slate-950/70 px-3 py-1.5 rounded-lg border border-amber-500/20 self-end sm:self-auto">
                  合計 18回 のドライバーズタイトル
                </div>
              </div>
      ...
      {viewMode === 'legends' && (
        <div className="flex flex-col gap-4">
          <div className="bg-gradient-to-r from-amber-500/10 via-amber-600/5 to-transparent border border-amber-500/30 rounded-2xl p-4 sm:p-6 shadow-xl">
            <div className="flex items-center gap-3">
