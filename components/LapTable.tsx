 ...
 * Tabbed lap detail table per driver.
 */

import React, { useState, useMemo } from 'react';
import type { Driver, Lap, Stint } from '@/lib/types';
import { enrichLapsWithStints, formatLapTime, formatSectorTime, getTyreColor } from '@/lib/telemetryUtils';
...
export default function LapTable({ selectedDrivers, lapsCache, drivers, stints }: LapTableProps) {
  const [activeTab, setActiveTab] = useState<string | null>(selectedDrivers[0] ?? null);

  // Keep active tab in sync when drivers change
  const currentTab = selectedDrivers.includes(activeTab ?? '') ? activeTab : selectedDrivers[0] ?? null;
  ...
  // Find best lap for gold highlight
  const validTimes = laps.map(l => l.lap_duration ?? l.lap_time ?? 0).filter(t => t > 0);
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {sorted.map(lap => {
                ...
                const isPit = lapTime !== null && lapTime !== undefined && lapTime > 110;
                ...
                return (
                  ...
                  >
                    <td className={`px-3 py-1.5 font-bold ${isBest ? 'text-purple-300' : 'text-slate-200'}`}>
                      {lap.lap_number}
                      {isBest && <span className="ml-1 text-purple-400">★</span>}
                    </td>
                    <td className={`px-3 py-1.5 font-racing font-medium ${isBest ? 'text-purple-300' : isPit ? 'text-slate-500' : 'text-white'}`}>
                      {lapTime ? formatLapTime(lapTime) : 'PIT'}
                    </td>
                    <td className="px-3 py-1.5 text-slate-300">{formatSectorTime(lap.duration_sector_1)}</td>
                    <td className="px-3 py-1.5 text-slate-300">{formatSectorTime(lap.duration_sector_2)}</td>
                    <td className="px-3 py-1.5 text-slate-300">{formatSectorTime(lap.duration_sector_3)}</td>
                    ...
                    <td className="px-3 py-1.5 text-slate-400">{lap.stintNumber ?? '-'}</td>
